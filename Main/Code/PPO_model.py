...
from mlp import MLPCritic, MLPActor, MLPIndiv
...
class HGNNScheduler(nn.Module):
    def __init__(self, model_paras, actor, critic):
        ...
        default_critic = MLPCritic(self.n_hidden_critic, self.critic_dim, self.n_latent_critic, 1).to(self.device)
        
        self.actor = default_actor if actor==None else actor
        self.critic = default_critic if critic==None else critic
...
class PPO:
    def __init__(self, model_paras, train_paras, actor, critic, num_envs=None):
        ...
        self.policy = HGNNScheduler(model_paras, actor, critic).to(self.device)
